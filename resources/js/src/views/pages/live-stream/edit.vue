        ...
        <p class="pt-1">
            ...
            <b-form @submit.prevent="onSubmit">
                ...
                <validation-observer ref="refObsForm">
                    ...
                    <b-media class="mb-2">
                        <template #aside>
                        <b-avatar :src="formData.thumbnail" :text="avatarText('NA')" size="90px" rounded />
                        </template>
                        <div class="d-flex flex-wrap">
                        <b-button v-if="hasPermission('live-stream-add-or-edit')" variant="primary" @click="$refs.refInputEl.click()">
                    ...
                    <b-row>
                        <!-- Live Stream Title -->
                        <b-col cols="12" md="4">
                            <validation-provider #default="{ errors }" name="Live Stream Title" vid="title" rules="required">
                              <b-form-group label="Live Stream Title" :state="errors.length > 0 ? false : null">
                                <b-form-input v-model="formData.title" :state="errors.length > 0 ? false : null" trim />
                    ...
                    <b-row>
                        <!-- Live Stream Link -->
                        <b-col cols="12" md="4">
                            <validation-provider #default="{ errors }" name="Live Stream Link" vid="link" rules="required">
                              <b-form-group label="Live Stream Link">
                                <b-form-input v-model="formData.link" :state="errors.length > 0 ? false : null" trim />
                    ...
                    <div class="d-flex mt-2">
                        <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mr-2"
                            type="submit" v-if="hasPermission('live-stream-add-or-edit')" :disabled="isLoading">
                            <b-spinner v-show="isLoading" small></b-spinner> Save Changes
                        </b-button>
...
import { getDetail } from '@/network/live-stream'
import { ValidationProvider, ValidationObserver } from 'vee-validate'
import { required, numeric } from '@validations'
import Ripple from 'vue-ripple-directive'
import vSelect from 'vue-select'
import { avatarText } from '@core/utils/filter'
import { postData } from '@/network/live-stream'
...
export default {
    ...
    methods: {
        inputImageRenderer() {
          this.formData.file_thumbnail = this.$refs.refInputEl.files[0]
        ...
        onSubmit() {
            this.$refs.refObsForm.validate().then(success => {
                ...
                postData(vForm).then(response => {
                    ...
                    this.$bvToast.toast('Live Stream has been changed successfully', {
                        title: `Success`,
                        variant: 'primary',
    ...
    data() {
        ...
        return {
            formData, required, numeric, isLoading: false,
        }
    }
}
</script>
