<template>
  <b-sidebar id="add-new-sidebar" :visible="isAddSidebarActive" bg-variant="white" sidebar-class="sidebar-lg" shadow
    backdrop no-close-on-backdrop no-header right @hidden="resetSocialMediaData"
    ...
    <template #default="{ hide }">
      ...
      <validation-observer ref="refObsForm">
        <!-- Form -->
        <b-form class="p-2" @submit.prevent="onSubmit" @reset.prevent="resetSocialMediaData">
          ...
          <validation-provider #default="{ errors }" name="Social Media Name" vid="social_media_name" rules="required">
            <b-form-group label="Social Media Name">
              <b-form-input v-model="formData.social_media_name" name="social_media_name"
          ...
          <validation-provider #default="{ errors }" name="Social Media Link" vid="social_media_link" rules="required">
            <b-form-group label="Social Media Link">
              <b-form-textarea id="social_media_link" v-model="formData.social_media_link"
                :state="errors.length > 0 ? false : null" trim />
          ...
          <!-- Icon -->
          <validation-provider #default="{ errors }" vid="image" name="Icon" rules="required">
            <b-form-group label="Icon" description="Recommended Size 300 x 300 px (Format : PNG, JPG, WEBP) Max Size: 1.5MB">
              <b-form-file accept="image/jpeg, image/png, image/webp" v-model="formData.file_icon"
    ...
    onSubmit() {
      this.$refs.refObsForm.validate().then(success => {
        ...
        postData(vForm).then(response => {
          this.$bvToast.toast(`Social Media has been added successfully`, {
            title: `Success`,
            variant: 'primary',
