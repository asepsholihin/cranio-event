<template>
    <b-card>
                    ...
                    <div class="row" v-if="transitForm">
                        <div class="col-12 border-bottom border-bottom-unset-height" v-for="(intent, key) in formData.hotel_event_list">
                            ...
                            <b-row>
                                ...
                                <b-col cols="12">
                                    <div class="row" v-for="(childs, index) in formData.hotel_event_list[key].child">
                                        <div class="col-12">
                                            <div class="d-flex justify-content-end mb-2">
                                                <b-button class="bg-transparent remove"
                                                    type="button" @click="removeChild(key, index)">
                                                    <feather-icon icon="TrashIcon" />
                                                </b-button>
                                            </div>
                                        </div>
        ...
        <!-- ADD RATE CATEGORY -->
        <b-modal size="lg" centered v-model="showAddRate" ok-title="Add" @ok="onSubmitCategory" @hidden="resetModal" no-close-on-backdrop>
...
import { postData, priceCategoryAll, postDataCategory, getDetail, deleteItem } from '@/network/master-hotel-event'
...
export default {
    ...
    methods: {
        ...
        removeChild(key, index){
            if(this.formData.hotel_event_list[key].child[index].id){
                this.deleteItemVal('child', key, this.formData.hotel_event_list[key].child[index].id, index);
            }else{
                this.formData.hotel_event_list[key].child.splice(index, 1);
            }
        },
        remove(type, key){
            if(type == 'hd'){
                if(this.formData.manasik_hd_list[key].id){
                    this.deleteItemVal(type, key, this.formData.manasik_hd_list[key].id);
                }else{
                    this.formData.manasik_hd_list.splice(key, 1);
                }
            }
            if(type == 'fd'){
                if(this.formData.manasik_fd_list[key].id){
                    this.deleteItemVal(type, key, this.formData.manasik_fd_list[key].id);
                }else{
                    this.formData.manasik_fd_list.splice(key, 1);
                }
            }
            if(type == 'ad'){
                if(this.formData.manasik_ad_list[key].id){
                    this.deleteItemVal(type, key, this.formData.manasik_ad_list[key].id);
                }else{
                    this.formData.manasik_ad_list.splice(key, 1);
                }
            }
            if(type == 'da'){
                if(this.formData.manasik_da_list[key].id){
                    this.deleteItemVal(type, key, this.formData.manasik_da_list[key].id);
                }else{
                    this.formData.manasik_da_list.splice(key, 1);
                }
            }
            if(type == 'he'){
                if(this.formData.hotel_event_list[key].is_show){
                    this.deleteItemVal(type, key, this.formData.hotel_event_list[key].id, '', this.formData.hotel_event_list[key].rate_category);
                }else{
                    this.formData.hotel_event_list.splice(key, 1)
                }
            }
        },
        deleteItemVal(type, key, id, lm="", rate_category=""){
            const vForm = new FormData();
            vForm.append('type', type)
            vForm.append('id', id)
            vForm.append('master_hotel_event_id', this.formData.id)
            if(type == 'he'){
                vForm.append('rate_category', rate_category);
            }
            this.$swal({
                title: `Delete Item ?`,
                ...
                if (result.value) {
                    deleteItem(vForm).then(response => {
                        if(type == 'hd'){
                            this.formData.manasik_hd_list.splice(key, 1);
                        }
                        if(type == 'fd'){
                        ...
                        if(type == 'child'){
                            if(lm!=''){
                                this.formData.hotel_event_list[key].child.splice(lm, 1);
                            }
                        }
                    })
                }
        ...
        onSubmit() {
            this.$refs.refObsForm.validate().then(success => {
                ...
                postData(this.formData).then(response => {
                    this.$bvToast.toast('Master Hotel has been updated successfully', {
                        title: `Success`,
                        variant: 'primary',
                        toaster: 'b-toaster-top-center',
                        solid: true,
                    })
                    this.refactData(this.formData.id);
                })
                .catch(error => {
                    if (error.response.data.errors) {
                        this.$refs.refObsForm.setErrors(error.response.data.errors)
        ...
        refactData(id){
            getDetail(id).then(response => {
                ...
                if(this.formData['is_manasik'] == 'yes'){
                    this.manasikForm = true
                }
                if(this.formData['is_transit'] == 'yes'){
                    this.transitForm = true
                }
            }).catch(error => {
                if (error.response.data.errors) {
    ...
    data() {
        const formData = {}
        const manasikForm = false
        const transitForm = false
