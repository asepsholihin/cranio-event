<template>
  <b-sidebar id="import-sidebar" :visible="isImportSidebarActive" bg-variant="white" sidebar-class="sidebar-lg" shadow
    ...
    <template #default="{ hide }">
      ...
      <!-- BODY -->
      <div class="m-2">
        <a href="" @click.prevent="exportParticipant()">Download Template Import Participant</a>
      </div>
...
import { importData, downloadExampleImport, exportParticipant } from '@/network/participant'
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.refObsForm.validate().then(success => {
        ...
        importData(vForm).then(response => {
          this.$bvToast.toast(`Participant has been imported successfully`, {
            title: `Success`,
            variant: 'primary',
    ...
    downloadTemplate() {
      window.open('spa/participant/download-example-import','_blank');
    },
    exportParticipant() {
      if(!this.umrohTripId) {
