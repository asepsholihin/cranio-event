            ...
            <b-form @submit.prevent="onSubmit">
                ...
                <validation-observer ref="refObsForm">
                        ...
                        <b-col cols="12" md="6">
                            ...
                            <validation-provider #default="{ errors }" name="Meta Title" vid="meta_title">
                                <b-form-group label="Meta Title">
                                    <b-form-input id="meta_title" v-model="formData.meta_title"
                                        :state="errors.length > 0 ? false : null" trim />
                            ...
                            <validation-provider #default="{ errors }" name="Meta Description" vid="meta_description">
                                <b-form-group label="Meta Description" description="Maximum : 300 Words">
                                    <b-form-textarea id="meta_description" v-model="formData.meta_description"
                                        @input="seoChecker" name="meta_description"
                    ...
                    <div class="d-flex mt-2">
                        <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="secondary" class="mr-2"
                            @click.prevent="onSaveAs('draft')" v-if="hasPermission('article-add-or-edit')">
                            Save as Draft
                        </b-button>
                        <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mr-2"
                            @click.prevent="onSaveAs('publish')" v-if="hasPermission('article-add-or-edit')">
                            Publish Article
                        </b-button>
                        <b-button @click.prevent="onSaveAs('preview')" v-ripple.400="'rgba(186, 191, 199, 0.15)'"
                            type="button" class="mr-2" variant="outline-primary">
                            Preview Article
                        </b-button>
...
<script>
import { BCard, BLink, BFormInvalidFeedback, BButton, BAvatar, BAlert, BForm, BRow, BCol, BFormGroup, BFormInput, BFormFile, BInputGroup, BInputGroupAppend, BFormCheckboxGroup, BFormCheckbox, BFormTextarea, BMedia } from 'bootstrap-vue'
        ...
        onSubmit() {
            this.$refs.refObsForm.validate().then(success => {
                ...
                postData(vForm).then(response => {
                    this.$bvToast.toast('Article has been created successfully', {
                        title: `Success`,
                        variant: 'primary',
                    ...
                    if (this.saveAs == "preview") {
                        ...
                        // var url = "http://localhost:4200/artikel/"+ response.data.slug + "?preview=true"
