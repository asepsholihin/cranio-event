<template>
  <div>
    ...
    <b-card no-body class="mb-0">
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              <b-form-input v-model="searchQuery" debounce="350" type="search" class="d-inline-block mr-1"
                placeholder="Search..." />
              <b-link class="btn btn-primary" :to="{ name: 'gallery-content-create' }"
                v-if="hasPermission('gallery-add-or-edit')">
                <span class="text-nowrap">Add Gallery</span>
              </b-link>
            </div>
          </b-col>
        </b-row>
      ...
      <b-table ref="refItemListTable" class="position-relative" :items="fetchItems" responsive hover
        :fields="tableColumns" primary-key="id" :sort-by.sync="sortBy" show-empty empty-text="No matching records found"
        :sort-desc.sync="isSortDirDesc">

        <!-- Column: Title -->
        <template #cell(title)="data">
          <img :src="data.item.image_url" height="80">
          <b-link :to="{ name: 'gallery-content-detail', params: { id: data.item.id } }"
        ...
        <template #cell(actions)="data">
          <b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL">
            ...
            <b-dropdown-item :to="{ name: 'gallery-content-detail', params: { id: data.item.id, title: data.item.title } }">
              <feather-icon icon="FileTextIcon" />
              <span class="align-middle ml-50">Details</span>
            </b-dropdown-item>
            <b-dropdown-item variant="danger" @click="deleteItem(data.item)">
...
import { deleteData } from '@/network/gallery-content'
