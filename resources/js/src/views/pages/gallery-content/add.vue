    ...
    <p>
      ...
      <validation-observer ref="refObsForm">
        ...
        <b-form class="p-2" @submit.prevent="onSubmit" @reset.prevent="resetItemData">
          ...
          <!-- Category -->
          <validation-provider #default="{ errors }" name="Select Category" vid="gallery_category_id" rules="required">
            <b-form-group label="Select Category" :state="errors.length > 0 ? false : null">
              <v-select v-model="formData.gallery_category_id" :options="categoryOptions" :clearable="false"
          ...
          <validation-provider #default="{ errors }" name="Content" vid="content">
            <b-form-group label="Content">
              <b-form-textarea id="content" v-model="formData.content" name="content"
          ...
          <validation-provider #default="{ errors }" vid="image" name="Image" rules="required">
            <b-form-group label="Image" description="Recommended Size 1080 x 800 px (Format : PNG, JPG, WEBP) Max Size: 1.5MB">
              <b-form-file accept="image/jpeg, image/png, image/webp" v-model="formData.file_image"
                ...
                drop-placeholder="Drop image here..." multiple="multiple" @change="onFileChange" />
          ...
          <b-row class="mb-2" v-if="images">
            <b-col class="position-relative" sm="3" v-for="(image, index) in images" :key="index">
              <img :src="image" class="img-fluid rounded" />
              <button class="btn btn-remove btn-danger btn-sm" @click="removeImage(index)"><feather-icon icon="Trash2Icon"/></button>
            </b-col>
...
<script>
import {
  BRow, BCol, BCard, BForm, BFormGroup, BFormInput, BFormTextarea, BFormFile, BFormInvalidFeedback, BButton, BAlert, BSpinner, BFormCheckbox
} from 'bootstrap-vue'
import { ValidationProvider, ValidationObserver } from 'vee-validate'
import { required, numeric } from '@validations'
import Ripple from 'vue-ripple-directive'
import vSelect from 'vue-select'
import { postData } from '@/network/gallery-content'
import { getCategoriesSearch } from '@/network/gallery-category'
  ...
  methods: {
    ...
    onSubmit() {
      this.$refs.refObsForm.validate().then(success => {
        ...
        postData(vForm).then(response => {
          ...
          this.$router.push({ name: 'gallery-content' })
        })
          .catch(error => {
    ...
    createImage(files) {
      ...
      for (var index = 0; index < files.length; index++) {
        ...
        this.imageFiles.push(files[index]);
      }
    },
    removeImage(index) {
