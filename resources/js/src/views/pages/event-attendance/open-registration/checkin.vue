    ...
    <div>
        <b-card>
            ...
            <b-row align-h="end">
                <b-col md="8">
                    <div class="d-flex">
                        <b-form-input v-model="scannedBarcode" autofocus name="barcode" ref="barcodeField" trim />
                        <b-button class="ml-1" variant="primary" @click="checkInBarcode" v-if="hasPermission('event-attendance-add-or-edit')" :disabled="loadingSeat">
                            <span class="text-nowrap"><b-spinner small v-show="loadingSeat" /> Check in</span>
                        </b-button>
                    </div>
                </b-col>
            </b-row>
                            ...
                            <ul class="list-group list-group-flush">
                                <template v-for="participant in participantList">
                                    <li class="list-group-item list-group-item-action" role="button" :class="{'active': participant.id == selectedParticipant.id}" @click="selectingParticipant(participant)">
                                        {{ participant.name }}<br>
                                        <span v-for="(seat, index) in participant.seat_codes" :key="index" class="text-uppercase">{{ seat }}, </span><br>
                                        <template v-if="participant.unchecked > 0">
                                            <span class="text-success">Absen: {{ participant.checked }}</span> <span class="font-weight-bold">|</span> <span class="text-danger">Belum: {{ participant.unchecked }}</span>
                                        </template>
                                        <template v-else>
                                            <span class="text-success">Sudah Absen Semua: {{ participant.checked }}</span>
                                        </template>
                                    </li>
                                </template>
                            </ul>
...
import { getDetail, getMappingSeatList, postCheckinEventOpenSeat, getSeatSettledList, getCheckinEventOpenSeatReport } from '@/network/event-open-registration'
...
export default {
    ...
    methods: {
        ...
        checkInBarcode() {
            ...
            vForm.barcode = this.scannedBarcode
            postCheckinEventOpenSeat(vForm).then(response => {
                this.loadingSeat = false
                this.$bvToast.toast('Nomor kursi berhasil checkin', {
                    title: `Success`,
                    variant: 'primary',
