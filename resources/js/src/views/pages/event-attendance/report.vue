...
export default {
    ...
    methods: {
        getChartData() {
            ...
            getChartAttendanceConfirmationByPackage({ params: { eventId: this.eventId, booking: this.bookingFilter, online: this.onlineManasik, session: this.sessionFilter } }).then(response => {
                var totalUnchecked = 0;
                for (const row of response.data.data_unchecked) {
                    totalUnchecked += row
                }
                this.chartUnconfirmedOptions = {
                    title: {
                        text: 'Total Belum Absen',
                        align: 'left'
                    },
                    chart: {
                        id: 'chart-attendance-unchecked',
                        toolbar: {
                            show: true
                        }
                    },
                    legend: {
                        ...
                        formatter: function(seriesName, opts) {
                            const value = opts.w.globals.series[opts.seriesIndex]
                            const percent = (value / totalUnchecked) * 100;
                            return seriesName + ":  " + percent.toFixed(1) +'%'
                        },
                    },
                ...
                this.chartUnconfirmedSeries = response.data.data_unchecked

                var totalChecked = 0;
                for (const row of response.data.data_checked) {
                    totalChecked += row
                }
                this.chartConfirmedOptions = {
                    title: {
                        text: 'Total Sudah Absen',
                        align: 'left'
                    },
                    chart: {
                        id: 'chart-attendance-checked',
                        toolbar: {
                            show: true
                        }
                    },
                    legend: {
                        ...
                        formatter: function(seriesName, opts) {
                            const value = opts.w.globals.series[opts.seriesIndex]
                            const percent = (value / totalChecked) * 100;
                            return seriesName + ":  " + percent.toFixed(1) +'%'
                        },
                    },
                    colors: response.data.colors,
                ...
                this.chartConfirmedSeries = response.data.data_checked
            }).catch(error => {
                this.$swal({ icon: 'error', title: 'Error', text: `${error.response.data.message}`, customClass: { confirmButton: 'btn btn-warning', }, buttonsStyling: false })
            })
