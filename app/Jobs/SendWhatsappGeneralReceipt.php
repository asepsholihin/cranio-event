...
class SendWhatsappGeneralReceipt implements ShouldQueue
{
    ...
    public function __construct($order)
    {
        App::setLocale('id');
        $this->order = $order;
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'receipt-general-payment')->first();
    ...
    public function handle()
    {
        try {
            ...
            $fileName = "kwitansi"."_".str_replace("/", "_", $this->order->invoice_no)."_".Carbon::now()->timestamp.".pdf";
            ...
            $header = new Header(
                Header::TYPE_DOCUMENT, $this->order->receipt_url, $fileName
            );
