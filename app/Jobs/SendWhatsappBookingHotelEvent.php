...
class SendWhatsappBookingHotelEvent implements ShouldQueue
{
    ...
    public function __construct($order)
    {
        App::setLocale('id');
        $this->order = $order;
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'link-booking-hotel-event-xendit')->first();
    ...
    public function handle()
    {
        try {
            ...
            $expire_date = Carbon::now()->addHours(24)->isoFormat('D MMMM Y HH:mm');
            ...
            $body = [
                new Body($this->name),
                new Body($this->order->invoice_description),
                new Body($this->order->hotel_name),
                new Body($this->order->room_type),
                new Body($this->order->total_room),
                new Body($this->order->additional_item),
                new Body("Rp " . number_format($this->order->total_amount, 0, ',', '.')),
                new Body($this->order->invoice_xendit_url),
                new Body($expire_date),
            ];
            ...
            LogQontakBroadcast::create([
                ...
                'type' => 'Link Xendit Pembayaran Booking Hotel Event',
                'status' => 'Delivered',
