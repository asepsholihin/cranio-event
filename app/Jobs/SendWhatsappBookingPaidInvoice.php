...
class SendWhatsappBookingPaidInvoice implements ShouldQueue
{
    ...
    public function __construct($order, $invoice)
    {
        ...
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'booking-order-paid-invoice')->first();
    ...
    public function handle()
    {
        try {
            ...
            $fileName = "invoice_".str_replace(" ", "_", $this->order->name)."_".str_replace("/", "_", $this->order->order_no)."_".Carbon::now()->timestamp.".pdf";
            if($transaction) {
                $fileName = 'invoice_'.$transaction->transaction_id."_".Carbon::now()->timestamp.'.pdf';
            }

            $umrohTrip = DB::table('umroh_trips')->select('departure_at')->where('id', $this->order->umroh_trip_id)->first();
            $maxPaidAt = Carbon::createFromFormat('Y-m-d', $umrohTrip->departure_at)->subDays(35);
            $tanggalJatuhTempo = $maxPaidAt->isoFormat('D MMMM Y');
            ...
            LogQontakBroadcast::create([
                ...
                'type' => 'Respon setelah bayar kirim invoice',
                'status' => 'Delivered',
