...
class SendWhatsappTicketEvent implements ShouldQueue
{
    ...
    public function __construct($event, $pax, $phoneNumber, $name, $barcode)
    {
        App::setLocale('id');
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'response-event-ticket-transaction')->first();
    ...
    public function handle()
    {
        try {
            ...
            $body = [
                ...
                new Body(Carbon::parse($this->event->event_at)->format("H:i") . " WIB"),
                // new Body(Carbon::parse($this->event->event_at)->subHour()->format("H:i") . " WIB"),
            ];
