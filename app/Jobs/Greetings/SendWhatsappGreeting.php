...
class SendWhatsappGreeting implements ShouldQueue
{
    ...
    public function __construct($order)
    {
        App::setLocale('id');
        $this->order = $order;
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'greetings')->first();
    ...
    public function handle()
    {
        ...
        $family = "Jejak Imani";
        $category = "Umroh";
        $categoryGelar = "Umroh yang maqbulah";
        $tim = "Tim Khidmat jejak imani";
        if($this->order->category_id == 2) {
            $category = "Haji";
            $categoryGelar = "Haji yang mabrur";
        }
        ...
        $body = [
            new Body($family),
            new Body($category),
            new Body($categoryGelar),
            new Body($tim)
        ];

        // [Optional] Create header message, support "DOCUMENT", "VIDEO", "IMAGE"
        $header = new Header(
            Header::TYPE_IMAGE, "www.jejakimani.com/assets/img/jejakimani/Grafis_Pelengkap_Greetings.jpg", "Grafis_Pelengkap_Greetings.jpg"
        );
        ...
        try {
            ...
            LogQontakBroadcast::create([
                ...
                'type' => 'Greetings setelah pembayaran DP awal',
                'status' => 'Delivered',
