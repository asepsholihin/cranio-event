...
class SendWhatsappBookingPaid implements ShouldQueue
{
    ...
    public function __construct($order, $invoice)
    {
        ...
        $qontakTemplate = DB::table('qontak_templates')->where('name', 'booking-order-paid-receipt')->first();
    ...
    public function handle()
    {
        try {
            ...
            $fileName = "kwitansi_".str_replace(" ", "_", $this->order->name)."_".str_replace("/", "_", $this->invoice->invoice_no)."_".Carbon::now()->timestamp.".pdf";
            ...
            if($transaction) {
                ...
                $fileName = 'kwitansi_'.$transaction->transaction_id."_".Carbon::now()->timestamp.'.pdf';
            }
            ...
            $header = new Header(
                Header::TYPE_DOCUMENT, $this->invoice->receipt_url, $fileName
            );
            ...
            LogQontakBroadcast::create([
                ...
                'type' => 'Respon setelah bayar kirim kwitansi',
                'status' => 'Delivered',
