...
class EquipmentDeliveryExport implements FromQuery, ShouldAutoSize, WithHeadings, WithStyles, WithMapping, WithEvents
{
    private $rowNumber;
    private $request;
    ...
    */
    public function map($equipmentDelivery): array
    {
        ...
        return [
            $this->rowNumber,
            $equipmentDelivery->departure_at,
            ($equipmentDelivery->packing_date) ? Carbon::parse($equipmentDelivery->packing_date)->isoFormat('D MMMM Y') : null,
            ($equipmentDelivery->pickup_date) ? Carbon::parse($equipmentDelivery->pickup_date)->isoFormat('D MMMM Y') : null,
            $equipmentDelivery->packageName,
            $equipmentDelivery->name,
            ($equipmentDelivery->gender == 1) ? 'Pria': 'Wanita',
    ...
    */
    public function query()
    {
        ...
        if (!empty($this->request['umroh_trip_id'])) {
            if($this->request['prosess-equipment']){
        ...
        if (!empty($this->request['package'])) {
            if($this->request['prosess-equipment']){
                if(!empty($this->request['umroh_trip_id'])){
                    $ids = EquipmentDelivery::dispactPackageId($this->request['umroh_trip_id'], $this->request['package']);
                    $query->whereIn('package_umroh_trips.id', $ids);
                }
            }else{
                $query->where('package_umroh_trips.id', $this->request['package']);
            }
        }
